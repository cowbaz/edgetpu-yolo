...
from utils import plot_one_box, Colors, get_image_ndarray_tensor
...
class EdgeTPUModel:
    ...
    def predict(self, image, image_path, save_img=True, save_txt=True):
        # logger.info("Detection Output Path {}".format(image_path))

        full_image, net_image, pad = get_image_ndarray_tensor(image, self.input_size[0])
    ...
    def process_predictions(
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                if save_img and len(det) != 0:  # Add bbox to image
                ...
                if save_txt and len(det) != 0:
                    bbox = {
                        "image_id": output_path.split("/")[-1],
                        "bbox": xyxy,
                        "conf": conf,
                        "cls": cls,
                        "cls_name": self.names[c],
                    }

                    output["annotations"].append(bbox)
            ...
            if len(det) != 0:
                logger.info("Output the detection file in {}".format(output_path))
