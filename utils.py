...
def get_image_ndarray_tensor(img, max_size, debug=False):
    """
    Reshapes an input image or ndarray image into a square with sides max_size
    """
    if isinstance(img, str) is str:
        img = cv2.imread(img)
    else:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # load ndarry image
